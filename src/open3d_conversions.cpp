...
// Minor modifications (c) 2022 Matt Young, UQ Racing Formula SAE Team
// - Cleaned up code
// - Applied clang-tidy recommendations (move constructors, static casts, etc)
...
// TODO fix all the compile warnings in this file
...
namespace open3d_conversions {
    void
    open3dToRos(const open3d::geometry::PointCloud &pointcloud, sensor_msgs::PointCloud2 &ros_pc2,
                std::string frame_id) {
        ...
        if (pointcloud.HasColors()) {
            ...
            // TODO use OMP for this for loop
            for (size_t i = 0; i < pointcloud.points_.size();
                ...
                *ros_pc2_x = static_cast<float>(point(0));
                *ros_pc2_y = static_cast<float>(point(1));
                *ros_pc2_z = static_cast<float>(point(2));
                *ros_pc2_r = (int) (255 * color(0));
                *ros_pc2_g = (int) (255 * color(1));
                *ros_pc2_b = (int) (255 * color(2));
            }
        } else {
            // TODO use OMP for this for loop (if enabled)
            for (size_t i = 0;
                 i < pointcloud.points_.size(); i++, ++ros_pc2_x, ++ros_pc2_y, ++ros_pc2_z) {
                const Eigen::Vector3d &point = pointcloud.points_[i];
                *ros_pc2_x = static_cast<float>(point(0));
                *ros_pc2_y = static_cast<float>(point(1));
                *ros_pc2_z = static_cast<float>(point(2));
    ...
    void rosToOpen3d(const sensor_msgs::PointCloud2ConstPtr &ros_pc2,
        ...
        if (ros_pc2->fields.size() == 3 || skip_colors) {
            ...
            if (ros_pc2->fields[3].name == "rgb") {
                ...
                for (size_t i = 0; i < ros_pc2->height * ros_pc2->width;
                     ++i, ++ros_pc2_x, ++ros_pc2_y, ++ros_pc2_z, ++ros_pc2_r, ++ros_pc2_g, ++ros_pc2_b) {
                    o3d_pc.points_.emplace_back(*ros_pc2_x, *ros_pc2_y, *ros_pc2_z);
                    o3d_pc.colors_.emplace_back(((int) (*ros_pc2_r)) / 255.0,
                                                ((int) (*ros_pc2_g)) / 255.0,
                                                ((int) (*ros_pc2_b)) / 255.0);
                }
                ...
                for (size_t i = 0; i < ros_pc2->height *
                                       ros_pc2->width; ++i, ++ros_pc2_x, ++ros_pc2_y, ++ros_pc2_z, ++ros_pc2_i) {
                    o3d_pc.points_.emplace_back(
                            Eigen::Vector3d(*ros_pc2_x, *ros_pc2_y, *ros_pc2_z));
                    o3d_pc.colors_.emplace_back(
    ...
    void open3dToRos(const open3d::t::geometry::PointCloud &pointcloud,
        ...
        for (int i = 0; i < t_num_fields - 1; ++i) {
            ...
            if (field_name == "xyz") {
                ...
                if ((field_name == "rgb") || (field_name == "rgba")) {
                    ...
                    if (data_type == "float") {
                        offset = addPointField(ros_pc2, field_name + "_x", 1,
                                               sensor_msgs::PointField::FLOAT32, offset);
                        offset = addPointField(ros_pc2, field_name + "_y", 1,
                                               sensor_msgs::PointField::FLOAT32, offset);
                        offset = addPointField(ros_pc2, field_name + "_z", 1,
                                               sensor_msgs::PointField::FLOAT32, offset);
                        offset += sizeOfPointField(sensor_msgs::PointField::FLOAT32);
                    } else if (data_type == "int") {
                        offset = addPointField(ros_pc2, field_name + "_x", 1,
                                               sensor_msgs::PointField::INT8, offset);
                        offset = addPointField(ros_pc2, field_name + "_y", 1,
                                               sensor_msgs::PointField::INT8, offset);
                        offset = addPointField(ros_pc2, field_name + "_z", 1,
                                               sensor_msgs::PointField::INT8, offset);
        ...
        for (auto &field_name: field_names) {
            ...
            if (field_name == "xyz") {
                ...
                const open3d::core::Tensor &o3d_TensorList_colors = pointcloud.GetPointAttr(
                        "colors");
                sensor_msgs::PointCloud2Iterator<uint8_t> ros_pc2_r(ros_pc2, "r");
                sensor_msgs::PointCloud2Iterator<uint8_t> ros_pc2_g(ros_pc2, "g");
                sensor_msgs::PointCloud2Iterator<uint8_t> ros_pc2_b(ros_pc2, "b");
                ...
                const open3d::core::Tensor &o3d_TensorList_fields = pointcloud.GetPointAttr(
                        field_name);
                if (data_type == "int") {
                    sensor_msgs::PointCloud2Iterator<uint8_t> ros_pc2_fx(ros_pc2,
                                                                         field_name + "_x");
                    sensor_msgs::PointCloud2Iterator<uint8_t> ros_pc2_fy(ros_pc2,
                                                                         field_name + "_y");
        ...
        for (int num_fields = 0; num_fields < ros_pc2->fields.size(); num_fields++) {
            if (ros_pc2->fields[num_fields].name == "x") {
                for (size_t i = 0; i < ros_pc2->height *
                    ...
                    o3d_TensorList_points.emplace_back(*ros_pc2_x, *ros_pc2_y, *ros_pc2_z);
                }
                ...
                for (size_t i = 0; i < ros_pc2->height *
                                       ros_pc2->width; ++i, ++ros_pc2_r, ++ros_pc2_g, ++ros_pc2_b) {
                    o3d_TensorList_colors.emplace_back(((int) (*ros_pc2_r)) / 255.0,
                                                       ((int) (*ros_pc2_g)) / 255.0,
                                                       ((int) (*ros_pc2_b)) / 255.0);
                }
                ...
                if (ros_pc2->fields[num_fields].datatype == sensor_msgs::PointField::UINT8 ||
                    ...
                    sensor_msgs::PointCloud2ConstIterator<uint8_t> ros_pc2_fx(*ros_pc2,
                                                                              ros_pc2->fields[num_fields].name);
                    sensor_msgs::PointCloud2ConstIterator<uint8_t> ros_pc2_fy(*ros_pc2,
                                                                              ros_pc2->fields[num_fields].name);
                    sensor_msgs::PointCloud2ConstIterator<uint8_t> ros_pc2_fz(*ros_pc2,
                                                                              ros_pc2->fields[num_fields].name);
                    std::vector<Eigen::Vector3d> o3d_TensorList_fields;

                    for (size_t i = 0; i < ros_pc2->height *
                                           ros_pc2->width; ++i, ++ros_pc2_fx, ++ros_pc2_fy, ++ros_pc2_fz) {
                        o3d_TensorList_fields.emplace_back(*ros_pc2_fx, *ros_pc2_fy, *ros_pc2_fz);
                    }
                ...
                } else if (ros_pc2->fields[num_fields].datatype ==
                           sensor_msgs::PointField::FLOAT32) {
                    sensor_msgs::PointCloud2ConstIterator<float> ros_pc2_fx(*ros_pc2,
                                                                            ros_pc2->fields[num_fields].name);
                    sensor_msgs::PointCloud2ConstIterator<float> ros_pc2_fy(*ros_pc2,
                                                                            ros_pc2->fields[num_fields].name);
                    sensor_msgs::PointCloud2ConstIterator<float> ros_pc2_fz(*ros_pc2,
                                                                            ros_pc2->fields[num_fields].name);
                    std::vector<Eigen::Vector3d> o3d_TensorList_fields;
